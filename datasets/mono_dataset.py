...
pil_loader        height
    ...
    def __getitem__(self, index):
        ...
        do_flip = self.is_train and (not self.opt.use_lidar) and (not self.opt.joint_pose) and random.random() > 0.5

        '''
        frame_index = self.filenames[index].strip().split()[0] #
        # frame_index = '15609121469313670' # 先选这张
        # '1569370965186787'还行
        # print(frame_index)
        self.get_info(inputs, frame_index, do_flip)
        '''

        if self.opt.rand_six_pic and self.is_train:
            input_six = {}
            for i in range(6):
                inputs = {}
                # frame_index_tmp = self.filenames[index].strip().split()[0] if i == 0 else self.filenames[(index+ random.randint(20,26729))%26729].strip().split()[0]
                frame_index_tmp = self.filenames[index].strip().split()[0] if i == 0 else self.filenames[(index + random.randint(20, 12318)) % 12318].strip().split()[0]
                self.get_info(inputs, frame_index_tmp, do_flip)
                input_six[i] = inputs
            for key in inputs:
                if key != 'id':
                    if isinstance(inputs[key],list):
                        inputs[key] = [input_six[j][key][j] for j in range(6)]
                    else:
                        inputs[key] = np.stack([input_six[j][key][j] for j in range(6)], axis=0)
            ...
            # try:
            #     self.get_info(inputs, frame_index, do_flip)
            #     inputs['image_is_empty'] = torch.Tensor([0])
            # except OSError:
            #     inputs = {}
            #     inputs['image_is_empty'] = torch.Tensor([1])
            #     return inputs
        ...
        if self.is_train:
            ...
            if self.opt.use_lidar:
                for j in range(len(inputs['lidar_depth'])):
                    inputs['lidar_depth'][j] = torch.from_numpy(inputs['lidar_depth'][j])
                # +Sfm
                if self.opt.dataset == 'nusc' and self.opt.use_sfm_loss:
