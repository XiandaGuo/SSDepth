...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        assert depth_est[mask] is not None, print('depth_est mask is bug!, check depthgt')
        assert depth_gt[mask] is not None, print('depth_gt mask is bug!, check depthgt')
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.get_depth  = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False),
                                              nn.Sigmoid())
        # self.get_depth  = nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False)
    ...
    def forward(self, features):
        ...
        self.outputs[("disp", 3)] = depth_8x8_scaled
        self.outputs[("disp", 2)] = depth_4x4_scaled
        self.outputs[("disp", 1)] = depth_2x2_scaled
        self.outputs[("disp", 0)] = final_depth
...
class encoder(nn.Module):
    def __init__(self, params):
        ...
        if params.encoder == 'densenet121_bts':
            ...
            self.base_model.load_state_dict(torch.load('/mnt/nas/algorithm/wenjie.yuan/pretrain/resnet50-0676ba61.pth'))
...
class BtsModel(nn.Module):
    def __init__(self, params):
        ...
        self.decoder = bts(params, self.encoder.feat_out_channels, 512)
        # self.decoder = bts(params, self.encoder.feat_out_channels, params.batch_size, 512)

    def forward(self, x):

        skip_feat = self.encoder(x)  # [6, 64, 176, 320],[6, 256, 88, 160],[6, 512, 44, 80],[6, 1024, 22, 40],[6, 2048, 11, 20]
...
if __name__ == '__main__':
    ...
    encoder_decoder = BtsModel(opts)
    encoder_decoder = (encoder_decoder).to(device)
    input = torch.rand(6, 3, 352, 640)
    # print(input)
    ...
    # print(output)
    print(np.unique(output[('disp', 0)].data.cpu().numpy()))
    print(np.unique(output[('disp', 1)].data.cpu().numpy()))
    print(np.unique(output[('disp', 2)].data.cpu().numpy()))
    print(np.unique(output[('disp', 3)].data.cpu().numpy()))

    print(output[('disp', 0)].shape)
    print(output[('disp', 1)].shape)
    print(output[('disp', 2)].shape)
    print(output[('disp', 3)].shape)
