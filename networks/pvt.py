...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            patch_embed = PatchEmbed(img_size=img_size // (2 ** i),
                                     patch_size=2,
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, (H, W) = patch_embed(x)  # [6, 3, 352,640] -> [6,14080,64]
...
class pvt_tiny(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_tiny, self).__init__(
            ...
            sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, pretrained=True)
...
#     def __init__(self, **kwargs):
#         super(pvt_small, self).__init__(
#             patch_size=4, embed_dims=[64, 64, 128, 320], num_heads=[1, 1, 2, 5], mlp_ratios=[8, 8, 8, 4],
#             qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 4, 6],
#             sr_ratios=[8, 8, 8, 4, 2], drop_rate=0.0, drop_path_rate=0.1, pretrained=True)
...
class pvt_small(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_small, self).__init__(
            patch_size=4, embed_dims=[32, 64, 96, 160, 224], num_heads=[1, 1, 2, 5, 8], mlp_ratios=[8, 8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 4, 6, 3],
            sr_ratios=[8, 8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, pretrained=True, num_stages=5)
        self.num_ch_enc = np.array([32, 64, 96, 160, 224])
# class pvt_small(PyramidVisionTransformer):
#     def __init__(self, **kwargs):
#         super(pvt_small, self).__init__(
#             patch_size=4, embed_dims=[32, 64, 128, 320, 512], num_heads=[1, 1, 2, 5, 8], mlp_ratios=[8, 8, 8, 4, 4],
#             qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 4, 6, 3],
#             sr_ratios=[16, 8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, pretrained=True, num_stages=5)
...
class pvt_small_f4(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_small_f4, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3],
            sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, F4=True, pretrained=True)
...
class pvt_medium(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_medium, self).__init__(
            ...
            sr_ratios=[8, 4, 2, 1], pretrained=True)
...
if __name__ == '__main__':
    ...
    # in_chans = 3
    # img = torch.randn(6, in_chans, h, w).to(device)
    #
    # model = pvt_small(in_chans=in_chans, patch_size=patch_size)
    #
    # model.to(device)
    # print(model)
    #
