...
class PixelFormer(nn.Module):
    ...
    def forward(self, imgs):  # imgs:kitti:[2, 3, 352, 1120]

        enc_feats = self.backbone(imgs)  # [2, 192, 88, 280], [2, 384, 44, 140], [2, 768, 22, 70], [2, 1536, 11, 35]
        if self.with_neck:
            enc_feats = self.neck(enc_feats)

        q4 = self.decoder(enc_feats) # [2, 512, 11, 35]

        q3 = self.sam4(enc_feats[3], q4) # [2, 1024, 11, 35]
        q3 = nn.PixelShuffle(2)(q3)      # [2, 256, 22, 70]
        ...
        bin_centers = self.bcp(q4)        # [2, 256, 1, 1]
        f = self.disp_head1(q0, bin_centers, 4)   # [2, 1, 352, 1120], equal to imgs' size
...
if __name__ == '__main__':
    ...
    in_chans = 3
    patch_size = 1
    # model = PixelFormer(version='large07', inv_depth=False, max_depth=80, pretrained='../pretrain/swin_large_patch4_window7_224_22k.pth')
    # model = PixelFormer(version='tiny07', inv_depth=False, max_depth=80, pretrained='../pretrain/swin_tiny_patch4_window7_224_22k.pth')
