...
class MHCA_stage(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # no CNN
        # att_outputs = []
        for x, encoder in zip(inputs, self.mhca_blks):  # [6, 64, 88, 160]
            # [B, C, H, W] -> [B, N, C]
            _, _, H, W = x.shape
            x = x.flatten(2).transpose(1, 2).contiguous()
...
class MPViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        outs.append(x)
        for idx in range(self.num_stages):
            att_inputs = self.patch_embed_stages[idx](x) # x:[6, 64, 176, 320]
            #outs.append(att_inputs)
            x,ff = self.mhca_stages[idx](att_inputs)
            outs.append(x)

        return outs # [6,64,176,320]
...
class MpvitEncoder(nn.Module):
    def __init__(self, layer_name, path):
        super(MpvitEncoder, self).__init__()
         # [6, 64, 176, 320],[6, 128, 88, 160],[6, 216, 44, 80],[6, 288, 22, 40],[6, 288, 11, 20]
        if layer_name == 'mpvit_tiny':
            self.encoder = self.mpvit_tiny()
            self.num_ch_enc = np.array([64, 96, 176, 216, 216])
        elif layer_name == 'mpvit_xsmall':
            self.encoder = self.mpvit_xsmall()
            self.num_ch_enc = np.array([64, 128, 192, 256, 256])

        if layer_name == 'mpvit_small':
            self.encoder = self.mpvit_small(path)
            if path == 1:
                self.num_ch_enc = np.array([64, 128, 320, 512, 512])
            elif path == 2:
                self.num_ch_enc = np.array([64, 128, 256, 320, 320])
            elif path == 3:
                self.num_ch_enc = np.array([64, 128, 192, 256, 256])
            elif path == 0:
                self.num_ch_enc = np.array([64, 128, 216, 288, 288])
            elif path == 4:
                self.num_ch_enc = np.array([64, 96, 176, 224, 224])
        if layer_name == 'mpvit_base':
            self.encoder = self.mpvit_base()
            self.num_ch_enc = np.array([128, 224, 368, 480, 480])

    def mpvit_tiny(self, **kwargs):
        """mpvit_tiny :
        ...
        checkpoint = torch.load('./pretrain/mpvit_tiny.pth', map_location=lambda storage, loc: storage)['model']
        load_state_dict(model, checkpoint, strict=False)
        model.default_cfg = _cfg_mpvit()
        return model


    def mpvit_xsmall(self, **kwargs):
        """mpvit_xsmall :
        ...
        checkpoint = torch.load('./pretrain/mpvit_xsmall.pth', map_location=lambda storage, loc: storage)['model']
        load_state_dict(model, checkpoint, strict=False)
        model.default_cfg = _cfg_mpvit()
        return model


    def mpvit_small(self, path, **kwargs):
        """mpvit_small :
        ...
        if path == 1:
            ...
            model = MPViT(
                ...
                num_path=[2, 4, 4, 4],
                num_layers=[1, 2, 4, 1],
                embed_dims=[64, 96, 176, 224],
                mlp_ratios=[4, 4, 4, 4],
                num_heads=[8, 8, 8, 8],
                **kwargs,
            )
        checkpoint = torch.load('/mnt/nas/algorithm/xianda.guo/intern/wenjie.yuan/pretrain/mpvit_small.pth', map_location=lambda storage, loc: storage)['model']
        load_state_dict(model, checkpoint, strict=False)
        model.default_cfg = _cfg_mpvit()
        return model


    def mpvit_base(self, **kwargs):
        """mpvit_base :
        ...
        checkpoint = torch.load('./pretrain/mpvit_base.pth', map_location=lambda storage, loc: storage)['model']
        load_state_dict(model, checkpoint, strict=False)
        model.default_cfg = _cfg_mpvit()
        return model

    def forward(self, input_image):
        self.features = self.encoder(input_image)
...
if __name__ == '__main__':
    ...
    model = MpvitEncoder('mpvit_small', 4)
